...
function displayData() {
  ...
  const objectStore = db.transaction("elite").objectStore("elite");
  objectStore.openCursor().addEventListener("success", (e) => {
    ...
    if (cursor) {
      ...
      quant.textContent = "QTY-" + cursor.value.qty;
      //calculate price x Qty to get current price
      calPrice = cursor.value.price.slice(3) * cursor.value.qty;
      pric.textContent = "\u20A6" + calPrice;

      //Loop through all price to get its content
      pric.classList.add("row");
      let rows = document.querySelectorAll(".row");
      let priceArray = Array.from(rows).map((items) => {
        return parseInt(items.innerHTML.slice(1));
      });
